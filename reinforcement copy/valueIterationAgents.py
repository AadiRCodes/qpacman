...
class ValueIterationAgent(ValueEstimationAgent):
            ...
            for s in self.mdp.getStates():
                ...
                for a in self.mdp.getPossibleActions(s):
                    ...
                    for (next, p) in self.mdp.getTransitionStatesAndProbs(s, a):
                        sum+=(p*(self.mdp.getReward(s, a, next)+self.discount*self.values[next]))
                    max_value = max(max_value, sum)
                if not self.mdp.isTerminal(s):
                    values_copy[s] = max_value
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for next, p in self.mdp.getTransitionStatesAndProbs(state, action):
            Q+=(p*(self.mdp.getReward(state, action, next)+self.discount*self.getValue(next)))
        return Q
